...
def main(): 
    """
    This function requires inputs from terminal to select initial
    image
    """
    question1 = input("Do you want to input an OME tiff?(y/n)")

    if question1 == "y":
        pyramidal = input("Insert name of pyramidal tiff:")
        pyramidal = Im.open(pyramidal)
        for i, page in enumerate(ImageSequence.Iterator(pyramidal)):
            page.save("images/Page%d.jpg" % i)
    
    #Page0 has the highest resolution
...
if __name__=='__main__':         # pragma: no cover
    ...
    image_name = "histo" #default
    image = Im.open("images/" + str(image_name)+".jpg")

# insert angle of rotation
angle = 0.0
...
def centering(step: int): 
    ...
    """
    n_cols = original_width // step # total number of columns and rows 
    n_rows= original_height// step
